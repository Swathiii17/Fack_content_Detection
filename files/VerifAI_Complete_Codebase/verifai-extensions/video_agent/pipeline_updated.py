# ─────────────────────────────────────────────────────────────────
# REPLACE backend/pipeline.py with this — adds video agent node
# ─────────────────────────────────────────────────────────────────
...
from agents.video_agent import video_agent_node      # NEW
...
async def parallel_agents_node(state: AnalysisState) -> AnalysisState:
    ...
    if "news"   in agents_to_run: tasks.append(news_agent_node(state))
    if "review" in agents_to_run: tasks.append(review_agent_node(state))
    if "image"  in agents_to_run: tasks.append(image_agent_node(state))
    if "audio"  in agents_to_run: tasks.append(audio_agent_node(state))
    if "video"  in agents_to_run: tasks.append(video_agent_node(state))   # NEW
    ...
    for result in results:
        ...
        if isinstance(result, dict):
            for key in ["news_result", "review_result", "image_result",
                        "audio_result", "video_result"]:              # NEW
